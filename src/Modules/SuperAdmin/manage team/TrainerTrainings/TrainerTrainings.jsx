...
import styles from "./TrainerTrainings.module.css";
...
export default function TrainerTrainings() {
  ...
  const { data: events, isLoading } = useFetchData({
    baseUrl: `${API_BASE_URL}admin/trainingManagement/?trainerId=${trainerId}`,
    queryKey: ["trainer-trainings", trainerId, queryFilters],
    params: queryFilters,
    token,
    options: { keepPreviousData: true, enabled: !!trainerId },
  });
  ...
  const columns = [
    {
      ...
      render: (row) => {
        return lang !== "ar"
          ? splitLang(row.event_eventName).en
          : splitLang(row.event_eventName).ar
          ? splitLang(row.event_eventName).ar
          : row.event_eventName || "-";
      },
    },
